...
from accounts.forms import PomodoroSettingsForm
...
class PomodoroSettingsFormTests(TestCase):
    def test_valid_settings(self):
        self.assertTrue(PomodoroSettingsForm(data=VALID).is_valid())

    def test_duration_below_minimum_is_invalid(self):
        form = PomodoroSettingsForm(data={**VALID, "pomodoro_focus": 0})
        self.assertFalse(form.is_valid())
        self.assertIn("pomodoro_focus", form.errors)

    def test_duration_above_maximum_is_invalid(self):
        form = PomodoroSettingsForm(data={**VALID, "pomodoro_short_break": 181})
        self.assertFalse(form.is_valid())
        self.assertIn("pomodoro_short_break", form.errors)

    def test_long_every_above_maximum_is_invalid(self):
        form = PomodoroSettingsForm(data={**VALID, "pomodoro_long_every": 13})
        self.assertFalse(form.is_valid())
